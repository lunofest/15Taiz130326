...
function updateTimer() {
    const targetDate = new Date("march 13, 2026 22:00:00").getTime();
...
(function () {
    ...
    const confirmBtn2 = document.querySelector('.lightbox__confirm2');
    const nombreInput = document.querySelector('#confirmationLightbox .lightbox__input');
    const mensajeInput = document.querySelector('.opcional__msj');
    const dietButtons = document.querySelectorAll('.lightbox__diet-btn');
    ...
    if (dietButtons.length > 0) {
        dietButtons.forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                restriccionSeleccionada = this.getAttribute('data-diet');
            });
        });
    }

    function enviarConfirmacion(numeroTelefono) {
        ...
        if (restriccionSeleccionada) {
            mensajeWhatsapp += `, Restricción alimentaria: *${restriccionSeleccionada}*`;
        }

        if (mensajeOpcional) {
            mensajeWhatsapp += `, Mensaje: *${mensajeOpcional}*`;
        }
    ...
    if (confirmBtn && nombreInput) {
        confirmBtn.addEventListener('click', function () {
            enviarConfirmacion('543812172129');
        });
    }

    if (confirmBtn2 && nombreInput) {
...
if (playlistBtn && lightboxPlaylist) {
    ...
    const whatsappNumber = '543812172129'; // Cambia aquí el número si lo necesitas
